...
class ClientReportController extends Controller
{
    ...
    public function addUpdate(Request $request)
    {
        $validator = \Validator::make($request->all(),[
            ...
            'trash.*.amount'=>['required_if:trash','between:1,99999999999'],
            'trash.*.company'=>['required_if:trash','numeric','exists:\App\Models\OtherCompany,id'],
            'trash.*.status'=>['nullable','in:Paid,Pending'],
            'trash.*.date'=>['required_if:trash','date_format:Y-m-d'],
            'supplies'=>['nullable','array'],
            'supplies.*.invoice'=>['required_if:supplies','max:50'],
            'supplies.*.company'=>['required_if:supplies','numeric','exists:\App\Models\OtherCompany,id'],
            'supplies.*.amount'=>['required_if:supplies','between:1,99999999999'],
            'supplies.*.status'=>['nullable','in:Paid,Pending'],
            'supplies.*.date'=>['required_if:supplies','date_format:Y-m-d'],
            'labour'=>['nullable','array'],
            'labour.*.precio_por_sq'=>['required_if:labour','between:1,99999999999'],
            'labour.*.company'=>['required_if:labour','numeric','exists:\App\Models\OtherCompany,id'],
            'labour.*.amount'=>['required_if:labour','between:1,99999999999'],
            'labour.*.status'=>['nullable','in:Paid,Pending'],
            'labour.*.date'=>['required_if:labour','date_format:Y-m-d'],
            'labour.*.deck'=>['required_if:labour','string'],
            'customer_payment'=>['nullable','array'],
            'customer_payment.*.amount'=>['required_if:customer_payment','between:0,999999999'],
            'customer_payment.*.date'=>['required_if:customer_payment','date_format:Y-m-d'],
            'inspection_result'=>['nullable','array'],
            'inspection_result.*.type'=>['required_if:inspection_result','max:191'],
            'inspection_result.*.date'=>['required_if:inspection_result','date_format:Y-m-d'],
            'seller_commission'=>['nullable','array'],
            'seller_commission.*.amount'=>['required_if:seller_commission','numeric','between:0,9999999999'],
            'seller_commission.*.paid'=>['required_if:seller_commission','numeric','between:0,9999999999'],
            'seller_commission.*.seller_id'=>['required_if:seller_commission','numeric','exists:\App\Models\User,id'],
            'seller_commission.*.status'=>['required_if:seller_commission','in:Paid,Pending'],
            'seller_commission.*.date'=>['required_if:seller_commission','date_format:Y-m-d'],
            'contract_price'=>['nullable','array'],
            'contract_price.*.label'=>['required_if:contract_price','string'],
            'contract_price.*.value'=>['required_if:contract_price','between:0,9999999999'],
            'contract_price.*.percent'=>['required_if:contract_price','between:0,9999999999'],
            'roofing_information'=>['nullable','array'],
            'roofing_information.*.deck'=>['required_if:roofing_information','string'],
            'roofing_information.*.perimeter'=>['required_if:roofing_information','between:0,999999999'],
            'roofing_information.*.area'=>['required_if:roofing_information','between:0,999999999'],
            'roofing_information.*.pitch'=>['required_if:roofing_information','string'],
            'wood_replace'=>['nullable','array'],
            'wood_replace.*.description'=>['required_if:wood_replace','string'],
            'wood_replace.*.measure'=>['nullable','string'],
            'wood_replace.*.unit'=>['required_if:wood_replace','between:0,999999999'],
            'wood_replace.*.quantity'=>['required_if:wood_replace','between:0,999999999'],
            'wood_replace.*.total'=>['required_if:wood_replace','between:0,999999999'],
            'wood_replace.*.discount'=>['required_if:wood_replace','between:0,999999999'],
            'wood_replace.*.collect'=>['required_if:wood_replace','between:0,999999999'],
            'city_for_permit'=>['nullable','array'],
            'city_for_permit.*.stage'=>['required_if:city_for_permit','string'],
            'city_for_permit.*.date'=>['required_if:city_for_permit','date_format:Y-m-d'],
            'city_for_permit.*.comment'=>['required_if:city_for_permit','string'],
        ]);
        if ($validator->fails()){
            $errors = "";
            $e = $validator->errors()->all();
